//
//  OppositeGradientHeaderView.swift
//  Final-Questor-App
//
...
@IBDesignable class OppositeGradientHeaderView: UIView {
    ...
    @IBInspectable var leftColor: UIColor = OppositeGradientHeaderView.hexStringToUIColor("#FF8B24") {
        didSet {
            updateUI()
        }
    }
    
    /// Bottom color of the gradient layer       // For some reason RGB values are not working.#FF350E
    @IBInspectable var rightColor: UIColor = OppositeGradientHeaderView.hexStringToUIColor("#FF340E"){//UIColor(red: 255.0, green: 136, blue: 35, alpha:1.0){
        didSet {
            updateUI()
        }
    }
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        setupGradientLayer()
        NSLog("the frame of opposite gradient view is %@", NSStringFromCGRect(frame));
    }
    ...
    func addQuestorLogo(){
        let image: UIImage = UIImage(named: "navlogo")!
        let imageView: UIImageView = UIImageView.init(image: image)
        imageView.frame = CGRect(x: 300, y: 30, width: image.size.width, height: image.size.height)
