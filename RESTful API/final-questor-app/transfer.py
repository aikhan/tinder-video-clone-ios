...
from streaming.buffered_stream import BufferedStream
from streaming.exceptions import CommunicationError
from streaming.exceptions import HttpError
from streaming.exceptions import TransferInvalidError
from streaming.exceptions import TransferRetryError
from streaming.http_wrapper import make_api_request
from streaming.http_wrapper import Request
from streaming.http_wrapper import RESUME_INCOMPLETE
from streaming.stream_slice import StreamSlice
from streaming.util import acceptable_mime_type
...
class _Transfer(object):
    ...
    def _ensure_initialized(self):
        ...
        :raises: :exc:`streaming.exceptions.TransferInvalidError`
...
class Download(_Transfer):
    ...
    def _set_range_header(request, start, end=None):
        ...
        :type request: :class:`streaming.http_wrapper.Request`
    ...
    def _get_chunk(self, start, end, headers=None):
        ...
        :rtype: :class:`streaming.http_wrapper.Response`
    ...
    def _process_response(self, response):
        ...
        :type response: :class:`streaming.http_wrapper.Response`
        :param response: response from a download request.
        :rtype: :class:`streaming.http_wrapper.Response`
        :returns: the response
        :raises: :exc:`streaming.exceptions.HttpError` for
                 missing / unauthorized responses;
                 :exc:`streaming.exceptions.TransferRetryError`
    ...
    def get_range(self, start, end=None, use_chunks=True):
        ...
        * start < 0, end = None: Fetch the last -start bytes of the file.
        ...
        :raises: :exc:`streaming.exceptions.TransferRetryError`
